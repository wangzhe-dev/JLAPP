<template>
	<PageLayout title="精度检测" :show-back="true" :sticky-toolbar="true">
		<!-- 顶部筛选 Tabs -->
		<template #toolbar>
...
async function request(params: {
	...
	let res: any;
	try {
		res = await http.post<any>("/quality/accuracyFill/findAllList", payload);
	} catch (e: any) {
...
async function beforeCloseFillPopout(type: "confirm" | "cancel" | "close") {
	...
	try {
		await postAction("/quality/accuracyFill/edit", payload);
		uni.showToast({ title: "保存成功", icon: "success" });
		fillPopoutItem.value = null;
		nextTick(() => listRef.value?.reload?.());
		return true;
	} catch (error) {
		console.error("[qualityAccuracy] save fill error", error);
...
async function deleteItem(item: any) {
	...
	const confirmed = await confirmModal({
		content: "是否确认删除该数据?",
		lockKey: "quality-accuracy-delete",
	});
	if (!confirmed) return;
	postAction("/quality/accuracyFill/delById", { id })
		.then(() => applyQuery())
		.catch((error: unknown) => {
...
function openDetail(item: any) {
	...
	if (!id) return;
	uni.navigateTo({
		url: `/pages/quality/jdFill?mode=view&id=${encodeURIComponent(id)}`,
	});
}
