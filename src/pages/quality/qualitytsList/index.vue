<template>
	<PageLayout title="探伤检测" :show-back="true" :sticky-toolbar="true">
		<!-- 顶部筛选 Tabs -->
		<template #toolbar>
...
async function request(params: {
	...
	let res: any;
	try {
		res = await http.post<any>("/quality/flawFill/findAllList", payload);
	} catch (e: any) {
...
function editItem(item: any) {
  console.log('33333',item);
	...
	uni.navigateTo({
		url: `/pages/exceptionManagement/exceptionReport/index?mode=edit&id=${encodeURIComponent(id)}`,
	});
}
...
async function beforeCloseFillPopout(type: "confirm" | "cancel" | "close") {
	...
	try {
		await postAction("/quality/flawFill/edit", payload);
		uni.showToast({ title: "保存成功", icon: "success" });
		fillPopoutItem.value = null;
		nextTick(() => listRef.value?.reload?.());
		return true;
	} catch (error) {
		console.error("[qualityFlaw] save fill error", error);
...
async function deleteItem(item: any) {
	...
	const confirmed = await confirmModal({
		content: "是否确认删除该数据?",
		lockKey: "quality-flaw-delete",
	});
	if (!confirmed) return;
	postAction("/quality/flawFill/delById", { id })
		.then(() => applyQuery())
		.catch((error: unknown) => {
